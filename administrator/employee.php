...
if($files=="" || $files==NULL){
$done=@mysql_query("insert into customers(customers_gender, customers_firstname, customers_lastname, customers_dob, customers_email_address, customers_telephone, id, barcode, title, wage, cash_drawer, cancel_order, void_order, checks, discount, daily_cal) values('$gender', '$fname', '$lname', '$birth', '$email', '$tel', '$ids', '$barcode', '$title', '$wage', '$cash_drawer', '$cancel_order', '$void_order', '$checks', '$discount', '$daily_cal')");
if($done){
...
$done=@mysql_query("insert into customers(customers_gender, customers_firstname, customers_lastname, customers_dob, customers_email_address, customers_telephone, id, barcode, title, wage, image, cash_drawer, cancel_order, void_order, checks, discount, daily_cal) values('$gender', '$fname', '$lname', '$birth', '$email', '$tel', '$ids', '$barcode', '$title', '$wage', '$test', '$cash_drawer', '$cancel_order', '$void_order', '$checks', '$discount', '$daily_cal')");
if($done){
@mysql_query("insert into address_book (customers_id, entry_firstname, entry_lastname,  entry_street_address, entry_city, entry_state, entry_postcode) values(LAST_INSERT_ID(), '$fname', '$lname', '$address', '$city', '$state', '$zip')");
}
...
<table width="798" border="0" align="center" cellpadding="0" cellspacing="0" class="text">
  ...
  <tr>
    ...
    <td colspan="4" valign="middle"><input name="barcode" type="text" class="text" id="barcode" size="30"   value="<?php echo $barcode;?>"/></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td height="29">&nbsp;</td>
    ...
    <td colspan="4" valign="middle"><input name="email" type="text" class="text" id="email" size="30"   value="<?php echo $email;?>"/>
      *</td>
    <td>&nbsp;</td>
    </tr>
  <tr>
    ...
    <td colspan="4" valign="middle"><input name="wage" type="text" class="text" id="wage" size="30"  value="<?php echo $wage;?>" />
      *</td>
    <td>&nbsp;</td>
    </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" valign="middle"><input type="checkbox" class="text" name="daily_cal" value="1" />
    ...
    <td colspan="12" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="text">
      ...
      <tr>
        <td width="196" height="25" valign="middle"><?php echo $employee['customers_firstname']." ".$employee['customers_lastname']; ?></td>
          <td width="171" valign="middle"><?php if($employee['title'])$show_emplyee=@mysql_query("select * from position where ids=".$employee['title']); 
		  
		  
		  
		  
		  if(mysql_num_rows($show_emplyee)>=1){ $row_employees=mysql_fetch_array($show_emplyee); echo $row_employees['title']; }?></td>
