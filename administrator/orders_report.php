...
if($_GET['action']=="resort"){
//resort====
if($month!=1){
$resort_time=$year."-".$month-1;
}else{
$resort_time=$year."-12";
}
$get_first_id=@mysql_query("select orders_id from orders where date_purchased like '%$resort_time%' order by orders_id desc");
if(mysql_num_rows($get_first_id)<=0){
$resort_time=$year."-".$month;
$get_first_id=@mysql_query("select orders_id from orders where date_purchased like '%$resort_time%' order by orders_id asc");
}

$first_id=mysql_fetch_array($get_first_id);
$_st_id=$first_id['orders_id'];
$empty_id="";
$empty_id=array();
$resort_time=$year."-".$month;
$nums_id=@mysql_query("select orders_id from orders where date_purchased like '%$resort_time%' order by orders_id asc");
$stop_id=@mysql_query("select orders_id from orders order by orders_id desc");
$row_stop=mysql_fetch_array($stop_id);


$h=0;
for($i=0; $i<=$row_stop['orders_id']; $i++){
 $tmp_order_id=$_st_id+$i;
$chk_order="";
$chk_order=@mysql_query("select orders_id from orders where orders_id='$tmp_order_id'");
//if($k==1){ $empty_id=$tmp_empty_id; }
if(mysql_num_rows($chk_order)<=0){
$empty_id[]=$tmp_order_id;
}

if($empty_id[$h]!="" && mysql_num_rows($chk_order)>=1){
$get_resort_id="";
$get_resort_id=mysql_fetch_array($chk_order);
$used_id="";
$used_id=$empty_id[$h];


@mysql_query("update orders set orders_id='$used_id' where orders_id=".$get_resort_id['orders_id']);
@mysql_query("update orders_products set parent_id='$used_id' where parent_id=".$get_resort_id['orders_id']);

$empty_id[]=$get_resort_id['orders_id'];
$h++;
}
}
//header("Location: ?content=orders_report&year=$year&month=$month");
//==========
...
$r1=@mysql_query("select DISTINCT(date_purchased) from orders where date_purchased like '%$search%' and orders_status=3 order by date_purchased asc");
...
$num_chk_07=3;
do{
if($hr_deduct>=0 && $num_chk_07>1 && $daily_deduct>0){
...
$daily_deduct=$daily_deduct-$deduct_value;
...
}while($num_chk_07>1 && $hr_deduct>0 && $daily_deduct>0);
}
...
<table width="800" border="0" cellpadding="0" cellspacing="0" align="center">
  ...
  <tr>
	...
	$t=1;
	$prev_hr="";
	$result=@mysql_query("select * from orders where date_purchased='$purchased' and orders_status=3 order by date_purchased asc");
  ...
  <tr>
    ...
    <td valign="middle" class="text"><input type="text" size="5"  name="deduct" class="text" /> <input type="submit" value="<?php echo DEDUCT;?>" class="text" /></td>
    <td colspan="3" align="right" valign="middle" class="text"> <input type="button" class="text" onclick="window.location='?content=orders_report&action=resort'" value="<?php echo RESORT;?>"/></td>
  </tr>
  ...
  <tr>
    ...
    <td align="center" valign="middle" class="text"><!--DWLayoutEmptyCell-->&nbsp;</td>
    <td align="center" valign="middle" class="text"><!--DWLayoutEmptyCell-->&nbsp;</td>
    <td colspan="3" align="right" valign="middle" class="text"><input type="button" class="text"  name="back_up" value="<?php echo BACK_UP;?>" onclick="window.location='export_order.php?year=<?php echo $year;?>&month=<?php echo $month; ?>'"/></td>
  </tr>
    </table>
    </td>
  </tr>
